...
struct Wavemaker : Module {
	...
	void process(const ProcessArgs& args) override {
		...
		float triangle = 4.f * fabsf(phase - 0.5f) - 1.f; // triangle wave
		float sawtooth = 2.f * phase - 1.f; // saw wave
		...
		if (inputs[PWM_INPUT].isConnected()) {
			pwm += inputs[PWM_INPUT].getVoltage() / 10.f;
		}
		pwm = clamp(pwm, 0.01f, 0.99f);

		float square = (phase > pwm) ? 1.f : -1.f; // square wave
		...
		float fc = clamp(4.f * freq, 20.f, nyquist * 0.99f);
		float alpha = 1.0f - std::exp(-2.0f * M_PI * fc * args.sampleTime);
		alpha *= 0.5f;

		float sawFiltered = sawFilterInit + alpha * (sawtooth - sawFilterInit);
		sawFilterInit = sawFiltered;

		float squareFiltered = squareFilterInit + alpha * (square - squareFilterInit);
		squareFilterInit = squareFiltered;
